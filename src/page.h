...
struct HasSpanInGetOutputKeyT<
    ...
    VoidT<decltype(std::declval<T>()
                       .get_output_key(
                           ...
                           std::declval<std::vector<cryptonote::output_data_t> &>()))>> : std::true_type
{
};
...
namespace xmreg
{
                ...
                for (const auto &field : tx_extra_fields)
                {
                    ...
                    if (!printer.get_stored_value().empty())
                    {
                        ...
                        if (wcnt == index)
                        {
                            ...
                            std::cout << "indx=" << wcnt << "txid=" << x << std::endl;
                        ...
                        else if (wcnt == index)
                        {
                            std::string x;
                            x = std::string{reinterpret_cast<const char *>(printer.get_stored_value().data()), printer.get_stored_value().size()};
                            std::cout "indx=" << wcnt << "sig=" << x << std::endl;
                            return std::string{reinterpret_cast<const char *>(printer.get_stored_value().data()), printer.get_stored_value().size()};
                        }
                    }
                }
    ...
    public:
        ...
        show_block(uint64_t _blk_height)
        {
            ...
            mstch::map context{
                ...
                {"is_randomx", (blk.major_version >= 12 && enable_randomx == true)},
            ...
            context["sum_fees"] = xmreg::xmr_amount_to_str(sum_fees, "{:0.6f}", false);

            // get xmr in the block reward
            context["blk_reward"] = xmreg::xmr_amount_to_str(txd_coinbase.xmr_outputs - sum_fees, "{:0.6f}");
        ...
        show_ringmembers_hex(string const &tx_hash_str)
        {
            ...
            for (txin_to_key const &in_key : input_key_imgs)
            {
                ...
                std::vector<uint64_t> absolute_offsets = cryptonote::relative_output_offsets_to_absolute(
                    in_key.key_offsets);
        ...
        show_ringmemberstx_hex(string const &tx_hash_str)
        {
            ...
            for (txin_to_key const &in_key : input_key_imgs)
            {
                ...
                for (auto const &txi : indices)
                {
                    ...
                    {
                        throw std::runtime_error("Cant get tx: " + pod_to_hex(mixin_tx_hash));
                    }
                    ...
                    all_mixin_txs[map_key].push_back(
                        pod_to_hex(mixin_tx_hash) + std::to_string(output_index_in_tx) + tx_hex);
                }
            ...
            if (enable_mixin_guess)
            {
                ...
                for (const txin_to_key &in_key : input_key_imgs)
                {
                    ...
                    for (const uint64_t &abs_offset : absolute_offsets)
                    {
                        ...
                        mstch::node &has_found_outputs = boost::get<mstch::map>(
            ...
            if (unsigned_tx_given)
            {
                ...
                if (r)
                {
                    ...
                    for (const ::tools::wallet2::tx_construction_data &tx_cd : exported_txs.txes)
                    {
                        ...
                        pair<mstch::array, double> mixins_timescales = construct_mstch_mixin_timescales(
        ...
        show_checkrawkeyimgs(string raw_data, string viewkey_str)
        {
            ...
            for (size_t n = 0; n < no_key_images; ++n)
            {
                ...
                crypto::key_image key_image = *reinterpret_cast<const crypto::key_image *>(record_ptr);

                crypto::signature signature = *reinterpret_cast<const crypto::signature *>(record_ptr + key_img_size);
        ...
        search(string search_text)
        {
            ...
            if (search_str_length == 95)
            {
                ...
                if (!xmreg::parse_str_address(search_text, address_info, nettype_addr))
                {
                    ...
                    return string("Cant parse address (probably incorrect format): ") + search_text;
                }
                            ...
                            const vector<pair<string, vector<string>>> &all_possible_tx_hashes)
        {
            ...
            for (const pair<string, vector<string>> &found_txs : all_possible_tx_hashes)
            {
                ...
                pair<mstch::map::iterator, bool> res = context.insert({found_txs.first, mstch::array{}});
                    ...
                    for (const string &tx_hash : found_txs.second)
                    {
                        ...
                        if (mcore->get_tx(tx_hash, tx))
                        {
                            ...
                            blk_height = core_storage
                                             ->get_db()
                                             .get_tx_block_height(tx_hash_pod);

                            blk_timestamp = core_storage
                                                ->get_db()
                                                .get_block_timestamp(blk_height);
                        }
                        else
                        {
        ...
        json_mempool(string _page, string _limit)
        {
            ...
            vector<MempoolStatus::mempool_tx> mempool_data = MempoolStatus::get_mempool_txs();
                     ...
                     bool tx_prove = false)
        {
            ...
            {
                j_response["status"] = "error";
                j_response["message"] = "Cant parse view key or tx private key: " + viewkey_str;
                return j_response;
            }
                           ...
                           bool in_mempool_aswell = false)
        {
            ...
            {
                j_response["status"] = "error";
                j_response["message"] = "Cant parse view key: " + viewkey_str;
                return j_response;
            }
            ...
            if (in_mempool_aswell)
            {
                ...
                // get mempool tx from mempoolstatus thread
                vector<MempoolStatus::mempool_tx> mempool_txs = MempoolStatus::get_mempool_txs();
    ...
    private:
                ...
                for (output_tuple_with_tag &outp : txd.output_pub_keys)
                {
                    ...
                    if (mine_output && tx.version == 2)
                    {
                        ...
                        if (!is_coinbase(tx))
                        {
                            ...
                            if (!r)
                            {
                                error_msg = "Cant decode ringct for tx: " + pod_to_hex(txd.hash);
                                return false;
                            }
        ...
        {
            // mark real mixing in the mixins timescale graph
            mstch::array &mixins_timescales = boost::get<mstch::array>(tx_context["timescales"]);
        ...
        {
            return template_file["header"] + middle + template_file["footer"];
        }

        bool
        get_monero_network_info(json &j_info)
        {
            MempoolStatus::network_info local_copy_network_info = MempoolStatus::current_network_info;
        ...
        get_footer()
        {
            ...
            static const mstch::map footer_context{
                ...
                {"git_branch_name", string{GIT_BRANCH_NAME}},
                {"monero_version_full", string{XCASH_VERSION_FULL}},
                {"api", std::to_string(ONIONEXPLORER_RPC_VERSION_MAJOR) + "." + std::to_string(ONIONEXPLORER_RPC_VERSION_MINOR)},
            };
